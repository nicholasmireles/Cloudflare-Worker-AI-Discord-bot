...
export async function sendChat(interaction, binding) {
  ...
  const username = usernameMappings[interaction.member.user.username] ?? usernameMappings[interaction.member.user.global_name] ?? interaction.member.user.username;
  const messages = [
    {role: 'system',
